...
| US-2.5 | En tant que partenaire, je veux créer un événement afin de le soumettre à la plateforme. | Créer le formulaire d’événement ; saisir les informations générales ; ajouter les médias ; définir la localisation et la date ; configurer les types de billets. | Must Have | 4 jours |
| US-2.6 | En tant que partenaire, je veux gérer mes événements afin de maintenir leurs informations à jour. | Lister les événements ; consulter les détails ; modifier un événement ; annuler ou archiver un événement ; gérer les types de billets associés. | Must Have | 3 jours |
...
Les acteurs concernés par ce sprint sont l’Utilisateur, le Partenaire et l’Administrateur. L’Utilisateur peut soumettre une demande pour devenir partenaire et suivre son statut. L’Administrateur intervient pour traiter les demandes partenaires et contrôler l’offre événementielle. Le Partenaire, après validation et activation de son accès, peut gérer son profil, créer des événements, gérer ses événements et configurer les types de billets associés.
...
| UC2.7 | Partenaire | Créer un événement | Saisir les informations de l’événement, ajouter les médias, définir la localisation, préciser la date et configurer les types de billets. |
...
| UC2.9 | Partenaire | Gérer les types de billets | Créer, consulter, modifier ou désactiver les types de billets associés à un événement. |
...
| Cas d’utilisation | Créer et gérer un événement |
| Acteur principal | Partenaire |
| Objectif | Permettre au partenaire de créer un événement et de gérer ses informations principales. |
| Précondition | Le partenaire est authentifié et son accès partenaire est activé. |
| Postcondition | L’événement est créé, modifié, annulé ou archivé selon l’action réalisée. |
| Scénario nominal | 1. Le partenaire accède à l’espace de gestion des événements.<br>2. Le système affiche la liste de ses événements.<br>3. Le partenaire choisit de créer un nouvel événement.<br>4. Il saisit les informations de l’événement.<br>5. Il ajoute les médias et définit la localisation.<br>6. Il configure les types de billets associés.<br>7. Le système vérifie les données saisies.<br>8. Le système enregistre l’événement.<br>9. Le système affiche une confirmation. |
| Scénarios alternatifs | Données invalides : le système affiche les erreurs de validation.<br>Types de billets manquants : le système demande de compléter la configuration.<br>Modification d’un événement : le partenaire met à jour les informations existantes.<br>Annulation ou archivage : le système met à jour le statut de l’événement. |
...
| Cas d’utilisation | Contrôler l’offre événementielle |
| Acteur principal | Administrateur |
| Objectif | Permettre à l’administrateur de vérifier les événements soumis afin de garantir leur conformité. |
| Précondition | L’administrateur est authentifié au back-office. |
| Postcondition | L’événement est approuvé, rejeté, mis en pause, signalé ou archivé. |
| Scénario nominal | 1. L’administrateur accède à l’interface de contrôle des événements.<br>2. Le système affiche les événements soumis ou à contrôler.<br>3. L’administrateur consulte le détail d’un événement.<br>4. Le système affiche les informations de l’événement.<br>5. L’administrateur choisit une action de contrôle.<br>6. Le système met à jour le statut de l’événement.<br>7. Le système affiche une confirmation. |
| Scénarios alternatifs | Événement non conforme : l’administrateur rejette ou met en pause l’événement.<br>Informations insuffisantes : l’administrateur signale l’événement ou demande une correction.<br>Erreur de mise à jour : le système affiche un message d’erreur. |
...
Le diagramme de classes du Sprint 2 présente les principales entités liées à la gestion des partenaires et des événements. Il complète le diagramme du sprint précédent en ajoutant les classes nécessaires à la demande partenaire, au profil partenaire, à l’événement, aux types de billets et au contrôle administratif.

La classe `Utilisateur` reste la classe de base du système. Lorsqu’une demande partenaire est acceptée, l’utilisateur peut accéder au rôle de partenaire. La classe `DemandePartenaire` représente le dossier soumis par l’utilisateur afin de devenir organisateur. La classe `ProfilPartenaire` regroupe les informations professionnelles du partenaire. La classe `Evenement` représente les événements créés par le partenaire, tandis que la classe `TypeBillet` permet de définir les catégories de billets associées à chaque événement.

Le diagramme contient également des énumérations permettant de représenter les statuts des demandes partenaires, des événements et des types de billets.
...
| Diagramme de séquence 4 | Créer un événement | Représenter la création d’un événement par le partenaire et la configuration des types de billets. |
| Diagramme de séquence 5 | Contrôler l’offre événementielle | Représenter le contrôle d’un événement par l’administrateur avant ou après sa publication. |

**Tableau 4.9 : Diagrammes de séquence du Sprint 2**

#### 4.3.2.1 Diagramme de séquence « Soumettre une demande partenaire »

Ce diagramme présente le processus de soumission d’une demande partenaire. L’utilisateur accède au formulaire de candidature, saisit ses informations professionnelles et dépose les documents requis. Après vérification des données, le système enregistre la demande avec un statut en attente et affiche une confirmation.
...
#### 4.3.2.4 Diagramme de séquence « Créer un événement »

Ce diagramme présente le processus de création d’un événement par le partenaire. Le partenaire saisit les informations principales de l’événement, ajoute les médias, définit la localisation et configure les types de billets. Après validation, le système enregistre l’événement et affiche une confirmation.

![Diagramme de séquence - Créer un événement](images/sequence_creer_evenement_sprint2.png)

**Figure 4.6 : Diagramme de séquence « Créer un événement »**

#### 4.3.2.5 Diagramme de séquence « Contrôler l’offre événementielle »

Ce diagramme présente le contrôle de l’offre événementielle par l’administrateur. L’administrateur consulte les événements soumis ou à contrôler, ouvre le détail d’un événement puis choisit une action de contrôle. Le système met ensuite à jour le statut de l’événement et affiche une confirmation.

![Diagramme de séquence - Contrôler l’offre événementielle](images/sequence_controler_offre_evenementielle_sprint2.png)

**Figure 4.7 : Diagramme de séquence « Contrôler l’offre événementielle »**
...
**Figure 4.8 : Interface de demande partenaire**
...
**Figure 4.10 : Interface d’activation partenaire**
...
**Figure 4.11 : Interface de profil partenaire**

### 4.4.5 Interface de création d’événement

Cette interface permet au partenaire de créer un nouvel événement. Il peut saisir les informations générales, définir la date, la localisation, ajouter les médias et configurer les types de billets associés. Après validation, l’événement est enregistré et pourra être soumis au contrôle administratif.

![Interface de création d’événement](images/interface_creation_evenement_sprint2.png)

**Figure 4.12 : Interface de création d’événement**

### 4.4.6 Interface de gestion des événements

L’interface de gestion des événements permet au partenaire de consulter la liste de ses événements et d’accéder à leurs détails. Elle permet également de modifier les informations d’un événement, de gérer ses types de billets et de changer son état selon les règles définies.

![Interface de gestion des événements](images/interface_gestion_evenements_sprint2.png)

**Figure 4.13 : Interface de gestion des événements**
...
**Figure 4.14 : Interface de contrôle de l’offre événementielle**
...
Nous avons ensuite présenté la conception du sprint à travers le diagramme de classes et les diagrammes de séquence relatifs à la demande partenaire, au traitement administratif, à l’activation partenaire, à la création d’événement et au contrôle de l’offre événementielle. Enfin, nous avons exposé les principales interfaces réalisées.
