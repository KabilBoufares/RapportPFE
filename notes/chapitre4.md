...
Ce sprint introduit également le contrôle de l’offre événementielle par l’administrateur afin de garantir la conformité des événements soumis avant leur publication sur la plateforme.
...
|---|---|---|---|---|
| US-2.1 | En tant qu’utilisateur, je veux soumettre une demande pour devenir partenaire afin de pouvoir organiser des événements. | Créer le formulaire de candidature ; saisir les informations professionnelles ; téléverser les documents requis ; envoyer la demande ; suivre son statut. | Must Have | 3 jours |
...
| US-2.5 | En tant que partenaire, je veux créer un événement afin de le soumettre au contrôle administratif. | Créer le formulaire d’événement ; saisir les informations générales ; ajouter les médias ; définir la localisation et la date ; choisir le mode de billetterie et configurer les types de billets internes si nécessaire. | Must Have | 4 jours |
| US-2.6 | En tant que partenaire, je veux gérer mes événements afin de maintenir leurs informations à jour. | Lister les événements ; consulter les détails ; modifier un événement ; annuler ou archiver un événement ; gérer les types de billets internes ou le lien externe selon le mode choisi. | Must Have | 3 jours |
| US-2.7 | En tant qu’administrateur, je veux contrôler l’offre événementielle afin de garantir la conformité des événements soumis avant leur publication. | Consulter les événements soumis ; afficher les détails ; approuver, rejeter, mettre en pause, signaler ou archiver un événement. | Must Have | 3 jours |
...
Le cas d’utilisation « Devenir partenaire organisateur » couvre principalement la soumission de la demande partenaire. Le suivi de la demande est une action possible pour l’utilisateur.

La finalisation de l’activation partenaire reste conditionnelle après l’acceptation de la demande par l’administrateur. Cette activation peut passer par l’onboarding Stripe. Le diagramme présente également la gestion des événements par le partenaire et le contrôle de l’offre événementielle par l’administrateur.
...
|---|---|---|---|
| UC2.1 | Utilisateur | Devenir partenaire organisateur | Soumettre une demande partenaire, suivre le statut de la demande et finaliser l’activation après acceptation. |
| UC2.2 | Utilisateur | Soumettre une demande partenaire | Remplir le formulaire de candidature, saisir les informations professionnelles, téléverser les documents requis et envoyer la demande. |
...
| UC2.4 | Utilisateur accepté comme partenaire | Finaliser l’activation partenaire | Compléter l’onboarding Stripe, vérifier le statut d’activation et accéder à l’espace partenaire après validation. |
...
| UC2.9 | Partenaire | Gérer les types de billets | Créer, modifier ou désactiver les types de billets lorsque l’événement utilise une billetterie interne. |
...
| Postcondition | La demande partenaire est enregistrée avec le statut SOUMISE. |
| Scénario nominal | 1. L’utilisateur accède à l’interface de demande partenaire.<br>2. Le système affiche le formulaire de candidature.<br>3. L’utilisateur saisit ses informations professionnelles.<br>4. Il téléverse les documents requis.<br>5. Le système vérifie les données saisies.<br>6. L’utilisateur soumet la demande.<br>7. Le système enregistre la demande avec le statut SOUMISE.<br>8. Le système affiche une confirmation de soumission. |
...
| Postcondition | La demande est acceptée, rejetée ou renvoyée pour complément d’informations, et son statut est mis à jour. |
| Scénario nominal | 1. L’administrateur accède à la liste des demandes partenaires.<br>2. Le système affiche les demandes SOUMISES.<br>3. L’administrateur consulte le détail d’une demande.<br>4. Le système affiche les informations et les documents associés.<br>5. L’administrateur vérifie le dossier.<br>6. Il accepte la demande.<br>7. Le système met à jour le statut de la demande à ACCEPTEE et le statut partenaire à APPROUVE.<br>8. Le système enregistre la décision, qui devient consultable par l’utilisateur dans le suivi de sa demande. |
...
Lorsque la demande est acceptée, son statut passe à ACCEPTEE et le statut partenaire passe à APPROUVE. L’accès complet sera activé après la finalisation de l’onboarding Stripe.
...
| Précondition | La demande partenaire est acceptée par l’administrateur et le statut partenaire est APPROUVE. |
| Postcondition | Le statut Stripe est vérifié, le partenaire devient ACTIF et l’accès à l’espace partenaire est activé. |
| Scénario nominal | 1. L’utilisateur accède à l’interface d’activation partenaire.<br>2. Le système vérifie que la demande est ACCEPTEE et que le statut partenaire est APPROUVE.<br>3. L’utilisateur lance l’onboarding Stripe.<br>4. Le système redirige l’utilisateur vers Stripe Connect.<br>5. L’utilisateur complète les informations demandées.<br>6. Le système vérifie le statut Stripe.<br>7. Le système met le statut partenaire à ACTIF et active l’accès partenaire.<br>8. L’utilisateur accède à son espace partenaire. |
...
| Précondition | Le partenaire est authentifié et son accès partenaire est activé. |
| Postcondition | L’événement peut être créé en brouillon, modifié, annulé ou soumis au contrôle administratif avec le statut SOUMIS et le statut de modération NON_CONTROLE. |
| Scénario nominal | 1. Le partenaire accède à l’espace de gestion des événements.<br>2. Le système affiche la liste de ses événements.<br>3. Le partenaire choisit de créer un nouvel événement.<br>4. Il saisit les informations principales de l’événement.<br>5. Il définit la localisation, la date, les médias et la catégorie.<br>6. Il choisit le mode de billetterie.<br>7. Si la billetterie est interne, il configure les types de billets.<br>8. Si la billetterie est externe, il renseigne le lien de billetterie.<br>9. Le système vérifie les données saisies.<br>10. Le système enregistre l’événement en brouillon.<br>11. Le partenaire soumet l’événement au contrôle administratif.<br>12. Le système met à jour l’événement avec le statut SOUMIS et le statut de modération NON_CONTROLE.<br>13. Le système affiche une confirmation. |
| Scénarios alternatifs | Données invalides : le système affiche les erreurs de validation.<br>Billetterie interne incomplète : le système demande de créer au moins un type de billet.<br>Billetterie externe incomplète : le système demande de renseigner le lien externe de billetterie.<br>Modification d’un événement : le partenaire met à jour les informations existantes.<br>Les modifications sensibles d’un événement déjà publié peuvent nécessiter un nouveau contrôle administratif.<br>Annulation d’un événement : le système met à jour le statut de l’événement. |
...
| Scénario nominal | 1. L’administrateur accède à l’interface de contrôle des événements.<br>2. Le système affiche les événements soumis et non encore contrôlés.<br>3. L’administrateur sélectionne un événement.<br>4. Le système affiche les informations détaillées de l’événement ainsi que son historique de modération.<br>5. L’administrateur examine les informations de l’événement.<br>6. L’administrateur approuve l’événement.<br>7. Le système met à jour le statut de l’événement à `PUBLIE` et le statut de modération à `APPROUVE`.<br>8. Le système affiche une confirmation de publication. |
...
Le diagramme de classes du Sprint 2 présente les principales entités liées à la gestion des partenaires et des événements. Il complète le sprint précédent en ajoutant les éléments nécessaires à la demande partenaire, aux documents de candidature, à l’activation partenaire, à l’événement, aux types de billets et au contrôle administratif.

La classe Utilisateur représente le compte de base de la plateforme. Les classes Partenaire et Administrateur spécialisent cette classe afin de représenter des rôles avec des responsabilités particulières. La classe Partenaire regroupe les informations professionnelles nécessaires à l’organisation des événements, comme le nom de l’entreprise, les liens sociaux, le statut partenaire et le statut Stripe. La classe Administrateur regroupe les opérations liées au traitement des demandes partenaires et au contrôle de l’offre événementielle.

La classe DemandePartenaire représente le dossier de candidature soumis par l’utilisateur. Les classes DocumentPartenaire et VerificationDocument modélisent les documents déposés et leur vérification.

La classe Evenement représente les événements créés par le partenaire. Elle contient les informations principales, le mode de billetterie, le lien externe éventuel ainsi que les statuts de publication et de modération. La classe TypeBillet représente les types de billets internes lorsque la billetterie est gérée dans FESTY. Enfin, la classe CategorieEvenement permet de classer les événements.
...
| Diagramme de séquence 6 | Contrôler l’offre événementielle | Représenter le contrôle d’un événement soumis par le partenaire avant sa publication. |

**Tableau 4.9 : Diagrammes de séquence du Sprint 2**

#### 4.3.2.1 Diagramme de séquence « Soumettre une demande partenaire »

Ce diagramme présente le processus de soumission d’une demande partenaire. L’utilisateur accède à l’espace de candidature, renseigne ses informations professionnelles et téléverse les documents requis. Le système permet également de reprendre un brouillon ou de compléter un dossier lorsque des informations supplémentaires sont demandées. Une fois le dossier complet, la demande est soumise et son statut est mis à jour afin d’être traitée par l’administrateur.
...
Ce diagramme présente la finalisation de l’activation partenaire après l’acceptation de la demande par l’administrateur. L’utilisateur accepté complète les étapes nécessaires à l’activation, notamment l’onboarding Stripe. Une fois le statut Stripe vérifié, le partenaire devient actif et l’accès à l’espace partenaire est autorisé.
...
Ce diagramme présente le processus de création et de gestion d’un événement par le partenaire. Le partenaire accède à son espace de gestion, crée un événement, choisit le mode de billetterie et configure les informations nécessaires. Lorsque la billetterie est interne, il peut ajouter les types de billets associés. Une fois les informations complètes, l’événement est soumis au contrôle administratif avec le statut SOUMIS et le statut de modération NON_CONTROLE. Le diagramme présente également, de manière simplifiée, les actions de modification et d’annulation d’un événement.

![Diagramme de séquence - Créer et gérer un événement](images/sequence_creer_evenement_sprint2.png)
...
Ce diagramme présente le processus de contrôle de l’offre événementielle par l’administrateur. Après avoir accédé à la file de modération, l’administrateur traite les événements soumis et non encore contrôlés. Il examine les informations et choisit une action de contrôle. Selon la décision prise, l’événement peut être approuvé, rejeté, mis en pause, signalé ou archivé. Le système met à jour les statuts correspondants et affiche une confirmation à l’administrateur.
...
L’interface de demande partenaire permet à un utilisateur authentifié de déposer une candidature afin de devenir partenaire organisateur. L’utilisateur saisit ses informations professionnelles, téléverse les documents requis et soumet sa demande pour traitement.
...
Cette interface permet au partenaire de créer un nouvel événement. Il peut saisir les informations générales, définir la date, la localisation, ajouter les médias et choisir le mode de billetterie. En mode interne, il configure les types de billets ; en mode externe, il renseigne le lien de billetterie. Après validation, l’événement est enregistré en brouillon. Il peut ensuite être soumis au contrôle administratif lorsque les informations requises sont complètes.
...
L’interface de gestion des événements permet au partenaire de consulter la liste de ses événements et d’accéder à leurs détails. Elle permet aussi de modifier un événement, de gérer les types de billets internes ou le lien externe de billetterie, puis de changer son état selon les règles définies.
...
Cette interface est utilisée par l’administrateur afin de contrôler les événements soumis sur la plateforme. Elle permet de consulter le détail d’un événement, puis d’approuver, rejeter, mettre en pause, signaler ou archiver l’événement selon son état.
...
Ce chapitre a présenté l’étude et la réalisation du Sprint 2, consacré à la gestion des partenaires et des événements.

Nous avons couvert le backlog, l’analyse fonctionnelle, la conception, les diagrammes UML retenus et les interfaces réalisées.

Ce sprint prépare les fonctionnalités suivantes, notamment la découverte des événements, les préférences utilisateur et les interactions autour des événements.